...
function createText() {
    inputStr = "<form action=\"http://www.w3schools.com/tags/demo_form.asp\">" +
        "<div align=\"center\" >" +
                "Text: <input type=\"text\"  id=" + IDControl + " title=" + IDControl + " name=" + IDControl + " maxlength=\"20\" autocomplete=on placeholder=" + IDControl + "  required onchange=\"UpdValueField()\"  textContent=\"abc\"/>" +
                "<input type=\"submit\">" +
                "</div>" +
                "</form>";
...
function AddItem_Select() {
    ...
    for (i = 0; i < _varItemCount; i++) {
        ...
        newOpt = new Option((_varItemName + "_" + i), (_varItemName + "_" + i));
        selectbox.options[i] = newOpt;
    }
}
...
function createParagraph(tagName) {
    ...
    //IDControl = tagName+"_ControlTest";
            ...
            if (tagName == "abbr") {
                ...
                inputStr = "<" + tagName + " title=" + IDControl + " data=\"http://www.logigear.vn/logi_media_dir/images/banners/flash1.swf\" name=" + IDControl + " id=" + IDControl + ">" + "</" + tagName + ">";
            }
...
function createSource() {
    ...
    if (navigator.appName != 'Microsoft Internet Explorer') {
        control.setAttribute('src', '../Video/movie.ogg');
        control.setAttribute('type', 'video/ogg');
    } else {
        control.setAttribute('src', '../Video/movie.mp4');
        control.setAttribute('type', 'video/mp4');
    }
}
...
function createInputSearch() {
    inputStr = "<form action=\"http://www.w3schools.com/tags/demo_form.asp\">" +
               "<input type=\"search\" id=" + IDControl + " autocomplete=on" + " placeholder=" + IDControl + "  required name=" + IDControl + " title=" + IDControl + ">" +
               "<input type=\"submit\">" +
               "</form>";
    document.write(inputStr);
    document.getElementById(IDControl).onchange = function () {
